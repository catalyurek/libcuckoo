...
private:
  ...
  public:
    ...
    inline void upgrade_to_x_lock() noexcept {
      assert( lock_ && !isXLocked() );
      unlock_shared();
      lock_x();
    }
  ...
  private:
    ...
    inline void unlock_x() { lock_.store(0); } // we need to ensure write to [elem_counter_] and [is_migrated_] visible if changed.
  ...
  using locked_table_mode_x = std::integral_constant<int, 3>;
  using locked_table_mode_shared = std::integral_constant<int, 2>;
  using normal_mode_x = std::integral_constant<int, 1>;
  using normal_mode_shared = std::integral_constant<int, 0>;
